...
#Criando time series 
df.ts <- ts(df.TS[,2:13], frequency = 12, start = c(2016,06))
ts.plot(df.ts, lty = 2)

#Verificando tendencia, sazonalidade e componente aleatorio 
decompose(df.ts[1:12])
...
pred1
#Cluster 1 pode ser de produtos vendidos tipicamente no inverno ou produtos descontinuados
...
#cluster7 <- Arima(df.ts[,7], order = c(1, 1, 0), include.drift = TRUE)
cluster7 <- Arima(df.ts[,7], order = c(1, 1, 0))
...
#Os produtos com previsao de 0 vendas, devem ter pelo menos 1 de estoque
Previsoes$`201706`[Previsoes$`201706` == 0] <- 1
